...
const sanitizeName = (value: string) => {
        ...
        return { normalized, extension } as const;
};
...
export const POST: RequestHandler = async ({ request, cookies }) => {
        ...
        const objectPath = `cases/${userData.user.id}/${Date.now()}-${normalized}${extension}`;

        const { error: uploadError } = await adminClient.storage.from('cases').upload(objectPath, fileBytes, {
        ...
        if (uploadError) {
                console.error('[cases/upload] Failed to store file', uploadError);
        ...
        if (publicUrlError || !publicUrlData?.publicUrl) {
                console.error('[cases/upload] Failed to generate public URL', publicUrlError);
