...
const computeScores = (bundle: AnalyzerBundle): CompositeScores => {
	...
	const authoritySignals = [
		...
		bundle.brokenLinks
			? -clamp(bundle.brokenLinks.checked.filter((entry) => !entry.ok).length * 10, 0, 30)
			: 0,
		bundle.divSoup ? (1 - bundle.divSoup.divShare) * 10 : 0
	];
	const authorityScore = clamp(
		45 + authoritySignals.reduce((sum, value) => sum + value, 0),
		0,
		100
	);
...
export const actions: Actions = {
	evaluate: async ({ request, fetch }) => {
		...
		try {
			...
			bundle.semanticDensity = await safeRun('semanticDensity', async () =>
				semanticDensity(mainHtml)
			);
			if (
