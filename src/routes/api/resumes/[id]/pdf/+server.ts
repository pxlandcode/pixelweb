...
export const GET: RequestHandler = async ({ params, url, cookies }) => {
        ...
        try {
                ...
                if (!accessToken) {
                        console.error('[pdf] Unauthorized: missing access token. Cookies present:', cookies.getAll().map(c => c.name));
                ...
                await page.waitForFunction(() => (document as any).fonts?.ready, { timeout: 5_000 }).catch(() => null);
