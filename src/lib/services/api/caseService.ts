...
const mapRecord = (record: Record<string, unknown>): CaseRecord => {
	...
	return {
		...
		imgAlt:
			...
			null,
		imagePosition: sanitizeImagePosition(imagePosition as 'left' | 'right' | null | undefined),
		bodyHtml:
			(source['bodyHtml'] as string | null | undefined) ??
			(source['body_html'] as string | null | undefined) ??
			null,
		showOnMainPage:
			(source['showOnMainPage'] as boolean | undefined) ??
			(source['show_on_main_page'] as boolean | undefined) ??
			false,
		status: (source['status'] as string | undefined) === 'published' ? 'published' : 'draft',
		displayOrder:
			(source['displayOrder'] as number | undefined) ??
			(source['display_order'] as number | undefined) ??
			1,
...
export const createCase = async (payload: CaseInput): Promise<CaseRecord | null> => {
	...
	try {
		const { data, error } = await client.from('cases').insert(sanitized).select('*').single();
