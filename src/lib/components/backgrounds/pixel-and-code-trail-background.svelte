<script lang="ts">
	...
	export let underlineThickness = 5;

	const PATH_BASE = 40;
	const TRAIL_COUNT = 4; // 👈 1 invisible + 3 visible (square → & → underline)

	let canvas: HTMLCanvasElement | null = null;
	let ctx: CanvasRenderingContext2D | null = null;
	let path2d: Path2D | null = null;

	type Kind = 0 | 1 | 2 | 3; // 0 = invisible, 1 = square, 2 = &, 3 = underline
	...
	function draw() {
		...
		// Add a new cell only when entering a new grid square
		if (gx !== lastGridX || gy !== lastGridY) {
			...
			trail.unshift({
				...
				cy: gy,
				t: now,
				kind: 0 // invisible head
			});

			// Assign kinds (0 = invisible, 1 = square, 2 = &, 3 = underline)
			for (let i = 0; i < trail.length; i++) {
				trail[i].kind = (i % TRAIL_COUNT) as Kind;
			}
		...
		// Remove faded cells
		trail = trail.filter(({ t }) => now - t < fadeSpeed);

		ctx.clearRect(0, 0, canvas.clientWidth, canvas.clientHeight);

		// Draw from oldest (tail) to newest (head)
		for (let i = trail.length - 1; i >= 0; i--) {
			const { cx, cy, t, kind } = trail[i];
			if (kind === 0) continue; // invisible lead cell
