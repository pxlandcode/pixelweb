<script lang="ts">
	...
	export let fadeSpeed = 2000;
	export let trailDuration = 8000;
	export let trailLength = 3;
	export let lagFactor = 0.1;
	...
	function draw() {
		...
		if (!last || last.cx !== cx || last.cy !== cy) {
			trail.push({ cx, cy, t: now });
		}

		trail = trail.filter(({ t }) => now - t < trailDuration);
		ctx.clearRect(0, 0, canvas.clientWidth, canvas.clientHeight);

		for (const { cx, cy, t } of trail) {
			const age = now - t;
			const phase = age / fadeSpeed;
			if (phase >= 1) continue;

			const life = 1 - Math.min(phase, 1);
			...
			if (phase < 0.33) {
				// --- Phase 1: white square ---
				ctx.fillStyle = squareColor;
				ctx.fillRect(px, py, gridSize, gridSize);
			} else if (phase < 0.66 && path2d) {
				// --- Phase 2: &
				ctx.save();
